#!/bin/sh
set -eu
if [ "$(uname -s)" != Darwin ]; then echo 'Speedrail packages currently support macOS.' >&2; exit 1; fi
case "$(uname -m)" in arm64) architecture=arm64;; x86_64) architecture=x64;; *) echo 'Unsupported Mac architecture.' >&2; exit 1;; esac
release_url=https://github.com/BerriAI/speedrail/releases/latest/download
scratch=$(mktemp -d "${TMPDIR:-/tmp}/speedrail-install.XXXXXX")
trap 'rm -rf "$scratch"' EXIT HUP INT TERM
echo 'Downloading Speedrail for your Mac…'
...
[ "$file" = "speedrail-$version-darwin-$architecture.tar.gz" ]
[ "${#checksum}" -eq 64 ]
case "$checksum" in *[!a-f0-9]*) echo 'Invalid release checksum.' >&2; exit 1;; esac
curl --fail --show-error --location --retry 2 --max-time 300 "https://github.com/BerriAI/speedrail/releases/download/v$version/$file" -o "$scratch/$file"
(cd "$scratch" && printf '%s  %s\n' "$checksum" "$file" | /usr/bin/shasum -a 256 -c -)
/usr/bin/tar -tzf "$scratch/$file" > "$scratch/files"
/usr/bin/awk '/^\// || /(^|\/)\.\.(\/|$)/ || !/^speedrail\// { bad=1 } END { exit bad }' "$scratch/files"
/usr/bin/tar -xzf "$scratch/$file" -C "$scratch"
"$scratch/speedrail/runtime/node" "$scratch/speedrail/bin/install.mjs" "$scratch/$file" "$scratch/manifest.json"
