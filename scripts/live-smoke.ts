...
const base=process.env.LITE_URL||'http://localhost:3210';
const model=process.env.LITE_TEST_MODEL;
if(!model)throw new Error('Set LITE_TEST_MODEL to an available coding model before running this live test.');
const workspace=await mkdtemp(join(tmpdir(),'lite-live-'));
...
try{
  ...
  await page.getByRole('textbox',{name:'Message Lite'}).fill('In this empty temporary workspace, read README.md, then create sum.mjs exporting function sum(a,b) returning a+b, and sum.test.mjs using node:test and node:assert/strict to test sum(2,3) === 5 and sum(-2,2) === 0. Run node --test sum.test.mjs via the bash tool. Do not install packages, use network, modify any other directory, or create more files. Finish with a concise report of the actual test result.');
  await page.getByRole('button',{name:'Send message',exact:true}).click();
  await page.getByRole('button',{name:'Stop generation'}).waitFor({timeout:15000});
  await page.screenshot({path:'.lite/live-streaming.png',fullPage:true});
  ...
  await page.screenshot({path:'.lite/live-complete.png',fullPage:true});
  ...
  await writeFile('.lite/live-smoke-result.json',JSON.stringify(summary,null,2));
