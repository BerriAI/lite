...
const install = await mkdtemp(join(tmpdir(), 'speedrail-production-tui-'));
...
try {
  ...
  await symlink(join(install, 'bin', 'speedrail.mjs'), join(install, 'global-bin', 'speedrail'));
  ...
  const env = { ...process.env, PATH: `${join(install, 'global-bin')}:${process.env.PATH}`, TERM: 'xterm-256color', SPEEDRAIL_PORT: String(port), SPEEDRAIL_MODEL: 'startup-smoke', SPEEDRAIL_DATA_DIR: join(install, 'state'), SPEEDRAIL_DISABLE_PROJECT_CONFIG: '1', XDG_CONFIG_HOME: join(install, 'config'), XDG_STATE_HOME: join(install, 'cache') };
  delete env.SPEEDRAIL_URL; delete env.SPEEDRAIL_TUI_CONFIG; delete env.SPEEDRAIL_CONFIG_DIR; delete env.SPEEDRAIL_WORKSPACE;
  terminal = pty.spawn('/bin/bash', ['--noprofile', '--norc', '-i'], { cwd: join(install, 'workspace'), cols: 100, rows: 30, name: 'xterm-256color', env: { ...env, PS1: 'SPEEDRAIL_SMOKE_SHELL> ' } });
  ...
  await waitFor(() => screen().includes('SPEEDRAIL_SMOKE_SHELL>'), 'Invoking shell did not open');
  terminal.write('speedrail\r');
  ...
  terminal.write('\x1a');
  await waitFor(() => screen().includes('Stopped') && screen().includes('SPEEDRAIL_SMOKE_SHELL>'), 'Suspend did not return to invoking shell');
  terminal.write('fg\r');
  ...
  await waitFor(() => screen().includes('SPEEDRAIL_SMOKE_SHELL>') && !screen().includes('Ctrl+P Commands'), 'Quit did not restore invoking shell');
  ...
  console.log('Production TUI passed: bare speedrail on PATH, automatic backend startup, caller workspace, suspend/foreground, clean exit, and backend ownership.');
} catch (error) {
