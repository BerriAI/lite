...
try {
  ...
  const env = { ...process.env, TERM: 'xterm-256color', LITE_PORT: String(port), LITE_MODEL: 'startup-smoke', LITE_DATA_DIR: join(install, 'state'), LITE_WORKSPACE: join(install, 'workspace'), LITE_DISABLE_PROJECT_CONFIG: '1', XDG_CONFIG_HOME: join(install, 'config'), XDG_STATE_HOME: join(install, 'cache') };
  delete env.LITE_URL; delete env.LITE_TUI_CONFIG; delete env.LITE_CONFIG_DIR;
  ...
  terminal.write([process.execPath, join(install, 'bin/lite.mjs'), 'tui'].map(quote).join(' ') + '\r');
  ...
  console.log('Production TUI passed: isolated install, automatic backend startup, caller workspace, suspend/foreground, clean exit, and backend ownership.');
} catch (error) {
