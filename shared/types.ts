...
export interface Message { id: string; sessionId: string; role: 'user' | 'assistant' | 'tool' | 'system'; content: string; reasoning?: string; toolCalls?: ToolCall[]; toolCallId?: string; createdAt: number; attachments?: Attachment[]; usage?: Usage; error?: string; }
...
export interface SessionDetail { session: Session; messages: Message[]; todos: Todo[]; permissions: PermissionRequest[]; }
...
export interface StreamChunk { type: 'text' | 'reasoning' | 'tool' | 'usage'; text?: string; tool?: { index: number; id?: string; name?: string; arguments?: string }; usage?: Usage; }
