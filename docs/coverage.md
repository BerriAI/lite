...
| Production runtime | Built server/static UI, persisted streaming tool workflow, CLI restart, native PTY, and exact undo/redo across restarts without provider replay | Opt-in isolated production test passed on exact Node 22.13.0, macOS arm64; BOM/UTF-8/CRLF/no-final-newline file bytes preserved |
| UI | React workspace, sessions, composer, model/settings/file/review/terminal/queue/history controls | 27 browser scenarios including streaming reload/isolation, permissions, queue/drafts, undo/redo, fork, inert import/export, terminal lifecycle, settings and responsive layouts; JSDOM race/storage regressions |
