...
Lite keeps every session's conversation in its local database. History search lets you — and the agent — find past decisions, commands, errors, and tool activity across all saved sessions on this machine, then read the surrounding transcript.
...
Search covers this machine's local Lite database only. Indexed content is capped per message part, snippets and context reads are bounded, and result counts are clamped — a search cannot dump unbounded history into the conversation. Exported session JSON is unaffected by indexing.
