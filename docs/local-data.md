...
- The web app's embedded shell terminals support macOS/Linux and live only while the server runs. Hidden terminals expire after 30 minutes with no viewers. Reconnection replays up to 256 KiB of output, not a full-screen terminal snapshot. Login profiles can add their own environment variables; the app does not forward its provider credentials. The full-screen `speedrail` client has separate [terminal lifecycle and platform notes](tui.md).
...
- `.env`, `.speedrail/`, logs, test outputs, and the local work log are ignored by Git. `.speedrail/` stores the local database and subscription tokens. Session exports may contain source code, paths, and tool output; review before sharing.
