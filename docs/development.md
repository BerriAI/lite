...
For a production build, run `npm run build`, then `npm start`. For the terminal, run `npm link` once and launch `lite` from your project directory. `npm link` points to this checkout; keep it in place. After pulling updates, run `npm install` and `npm run build` again, and restart any running Lite server to load the rebuilt backend.
