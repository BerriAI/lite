...
describe('independent profile acceptance and visibility audit', () => {
  ...
  let calls: unknown[];
  beforeEach(async () => {
    directory = await realpath(await mkdtemp(join(tmpdir(), 'lite-profile-audit-'))); calls = [];
    await mkdir(join(directory, '.lite/skills/check'), { recursive: true });
    await writeFile(join(directory, '.lite/profiles.json'), JSON.stringify({ version: 1, profiles: [{ id: 'review', name: 'Review', tools: ['read_file'], instructions: 'PRIVATE_PINNED_PROFILE', defaultModel: { providerId: 'primary', model: 'review-model' } }], skills: [{ id: 'check', name: 'Check' }] }));
    await writeFile(join(directory, '.lite/skills/check/SKILL.md'), 'PRIVATE_PINNED_SKILL');
  ...
  it.each([{ providerId: '', model: '' }, { providerId: '' }, { model: '' }])('rejects explicitly empty profile provider/model overrides %j', async pair => {
    await writeFile(join(directory, '.lite/profiles.json'), JSON.stringify({ version: 1, profiles: [{ id: 'writer', name: 'Writer', tools: ['read_file'] }], skills: [] }));
  ...
  it('a source changed before delayed resolution completes cannot be activated under an older catalog revision', async () => {
    ...
    await writeFile(join(directory, '.lite/skills/check/SKILL.md'), 'CHANGED_PINNED_SKILL'); release();
