...
describe('lifecycle hooks Runner/API integration', () => {
  ...
  beforeEach(async () => {
    directory = await realpath(await mkdtemp(join(tmpdir(), 'lite-hooks-runner-'))); store = new Store(join(directory, 'state')); calls = [];
  ...
  it('project hooks run only for a trusted workspace; untrusted gets the advisory', async () => {
    ...
    await writeFile(join(directory, '.lite', 'hooks.json'), JSON.stringify({ version: 1, hooks: [{ event: 'PreToolUse', command: 'exit 2', matcher: 'write_file' }] }));
