...
const sidekickCard = (page: Page) => page.getByRole('region', { name: 'Sidekick task', exact: true });
...
test('the sidekick writes behind parent-surfaced approvals and one persistent child spans turns', async ({ page, request }) => {
  ...
  await expect(sidekickCard(page)).toContainText('persistent delegated executor');
  ...
  await sidekickCard(page).getByRole('button', { name: 'Open transcript', exact: true }).click();
