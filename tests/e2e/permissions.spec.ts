...
test.beforeEach(async ({ page, request }) => {
  ...
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'lite-permissions-browser-'))); sessions = [];
...
test('project rules from .lite/permissions.json outrank app rules and deny without prompting', async ({ page, request }) => {
  expect((await rules(request, [{ tool: 'bash', decision: 'allow', patterns: ['touch *'] }])).ok()).toBe(true);
  await mkdir(join(workspace, '.lite'), { recursive: true });
  await writeFile(join(workspace, '.lite', 'permissions.json'), JSON.stringify({ version: 1, rules: [{ tool: 'bash', decision: 'deny', patterns: ['touch **'] }] }));
  const session = await create(request, { permissionMode: 'auto' }); await open(page, session);
...
test('an invalid project rules file is ignored with a visible notice and the turn still runs', async ({ page, request }) => {
  await mkdir(join(workspace, '.lite'), { recursive: true });
  await writeFile(join(workspace, '.lite', 'permissions.json'), '{ not valid json');
  const session = await create(request); await open(page, session);
