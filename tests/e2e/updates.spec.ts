...
const state = { currentVersion: '0.1.0', latestVersion: '0.2.0', available: true, packaged: true, restartRequired: false, releaseUrl: 'https://github.com/BerriAI/speedrail/releases/tag/v0.2.0', command: 'speedrail update' };
test('updates are visible, staged explicitly, and preserve work when restart is blocked', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'Speedrail updates' })).toContainText('Speedrail 0.1.0');
  await page.getByRole('button', { name: 'Install update', exact: true }).click();
  await expect(page.getByRole('region', { name: 'Speedrail updates' })).toContainText('0.2.0 is installed');
  await page.getByRole('button', { name: 'Restart', exact: true }).click();
  await expect(page.getByRole('region', { name: 'Speedrail updates' }).getByRole('alert')).toContainText('Finish active tasks');
});
test('source checkouts get an honest update path and offline checks can be retried', async ({ page }) => {
  ...
  const panel = page.getByRole('region', { name: 'Speedrail updates' });
