...
describe('notify unit (injected spawner, no real OS surface)', () => {
  ...
  it('macOS: builds one osascript -e argument with AppleScript-escaped strings and a 2s timeout', () => {
    setPlatform('darwin');
    const spawn = spawner();
    notify('Lite', 'He said "hi" \\ done', spawn);
    expect(spawn).toHaveBeenCalledTimes(1);
    const [command, args, options] = spawn.mock.calls[0];
    expect(command).toBe('osascript');
    expect(args[0]).toBe('-e');
    // \\ then " escaped — the only escapes AppleScript honors in string literals.
    expect(args[1]).toBe('display notification "He said \\"hi\\" \\\\ done" with title "Lite"');
  ...
  it('macOS: strips control and format characters so user text cannot smuggle terminal or script structure', () => {
    ...
    const spawn = spawner();
    notify('Lite', 'line1\nline2[2Jend‮', spawn);
    expect(spawn.mock.calls[0][1][1]).toBe('display notification "line1 line2 [2Jend " with title "Lite"');
  });
  it('linux: notify-send with plain title/body argv (no shell)', () => {
    setPlatform('linux');
    const spawn = spawner();
    notify('Lite', 'response finished', spawn);
    expect(spawn).toHaveBeenCalledWith('notify-send', ['Lite', 'response finished'], { timeout: 2000 }, expect.any(Function));
  });
  it('windows and unknown platforms: complete no-op', () => {
    for (const platform of ['win32', 'freebsd']) {
      setPlatform(platform);
      const spawn = spawner();
      notify('Lite', 'body', spawn);
      expect(spawn).not.toHaveBeenCalled();
    }
  });
  it('never throws, even when the spawner itself throws synchronously', () => {
    setPlatform('darwin');
    expect(() => notify('Lite', 'body', (() => { throw new Error('spawn failed'); }) as unknown as Spawner)).not.toThrow();
  });
...
describe('runner notification semantics (injected spawner)', () => {
  ...
  beforeEach(async () => {
    ...
    directory = await realpath(await mkdtemp(join(tmpdir(), 'lite-notify-runner-')));
