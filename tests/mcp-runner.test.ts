...
describe('immutable MCP turn lease and explicit lifecycle API integration',()=>{
  ...
  const realFixture=async()=>{
    ...
    store.saveSettings({mcpServers:{demo:{command:process.execPath,args:[script,log,catalog,notification]}}});realManager=new McpManager(()=>store.settings().mcpServers);const app=createApp({store,external:realManager});runner=app.runner;server=createServer(app.app);url=await listen(server);
  ...
  beforeEach(async()=>{
    directory=await realpath(await mkdtemp(join(tmpdir(),'lite-mcp-runner-')));store=new Store(join(directory,'state'));calls=[];captured=[];executions=[];generation=1;scopeValue='demo-schema-one';connected=true;releaseError=false;
  ...
  it('real manager API reconnect cannot redirect a tool already awaiting permission to replacement stdio server',async()=>{
    ...
    store.saveSettings({mcpServers:{demo:{command:process.execPath,args:[script,'original',counter]}}});realManager=new McpManager(()=>store.settings().mcpServers);const app=createApp({store,external:realManager});runner=app.runner;server=createServer(app.app);url=await listen(server);
    ...
    const previous=(await api('/settings')).body;expect((await api('/settings',{mcpServers:{demo:{command:process.execPath,args:[script,'replacement',counter]}},expectedMcpConfigRevision:previous.mcpConfigRevision},'PATCH')).status).toBe(200);
