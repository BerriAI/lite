...
describe('foreground bounded researcher Runner/API integration',()=>{
  ...
  beforeEach(async()=>{
    directory=await realpath(await mkdtemp(join(tmpdir(),'lite-delegation-runner-')));store=new Store(join(directory,'state'));calls=[];
  ...
  it('persists an independent read-only child and returns exactly one bounded report without MCP capture',async()=>{
    ...
    expect(names(calls[1]).sort()).toEqual(['glob','grep','read_file','todo_read','web_fetch']);expect(calls[1].messages.filter((m:any)=>m.role!=='system')).toEqual([{role:'user',content:'CHILD inspect'}]);expect(calls[1].model).toBe('model');expect(calls[1].messages[0].content).not.toContain('ROOT');
  ...
  it('Plan Auto authorizes launch without expanding the child read-only ceiling',async()=>{
    const s=await create({mode:'plan'});await run(s.id);expect(calls).toHaveLength(4);expect(runner.permissions(s.id)).toEqual([]);expect(names(calls[1]).sort()).toEqual(['glob','grep','read_file','todo_read','web_fetch']);expect(runner.delegations.list(s.id)[0].status).toBe('completed');
  });

  it('accepted provider, model, project guidance and skills survive source and settings replacement before child launch',async()=>{
    await mkdir(join(directory,'.lite','skills','inspect'),{recursive:true});await writeFile(join(directory,'.lite','skills','inspect','SKILL.md'),'Pinned skill body');await writeFile(join(directory,'.lite','profiles.json'),JSON.stringify({version:1,profiles:[],skills:[{id:'inspect',name:'Inspect',description:'Inspect project'}]}));
    ...
    store.saveSettings({providers:[{id:'test',name:'Replacement',kind:'openai',baseUrl:'http://127.0.0.1:1',apiKey:'fake-new-key'}]});await writeFile(join(directory,'AGENTS.md'),'Replacement guidance');await rm(join(directory,'.lite','skills','inspect','SKILL.md'));runner.decide(s.id,runner.permissions(s.id)[0].id,'allow');await runner.whenIdle();
