...
async function healthy(base) {
  ...
  if (!response.ok) throw new Error(`Lite returned HTTP ${response.status}.`);
  const value = await response.json();
  if (value.ok !== true || typeof value.version !== 'string') throw new Error('This address is not a Lite server.');
}
...
export async function ensureTuiServer({ base, root, workspace, explicit, env = process.env }) {
  ...
  if (!['localhost', '127.0.0.1'].includes(address.hostname) || address.pathname !== '/' || address.protocol !== 'http:') throw new Error('Start the server with lite serve, then attach with --url.');
  const directory = resolve(root, env.LITE_DATA_DIR || '.lite');
  ...
  if (!acquired) throw new Error('Another Lite server startup is in progress. Try again shortly.');
  try {
    ...
    const server = spawn(process.execPath, entry, { cwd: root, detached: true, stdio: ['ignore', fd, fd], env: { ...env, LITE_PORT: address.port || '80', LITE_WORKSPACE: workspace } });
    closeSync(fd);
    let failed;
    ...
    while (Date.now() < deadline) {
      if (failed || server.exitCode !== null) { server.kill('SIGTERM'); throw new Error(`Could not start Lite. Check ${log}.`); }
      try { await healthy(base); return { pid: server.pid, log }; } catch { /* Wait for the owned process to listen. */ }
      await delay(100);
    }
    server.kill('SIGTERM');
    throw new Error(`Lite did not become ready. Check ${log}.`);
