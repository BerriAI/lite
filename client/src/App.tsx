...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (session && session.id === currentId.current && !configBusy) setSelection({ providerId: session.providerId, model: session.model, mode: session.mode, permissionMode: session.permissionMode, planner: session.planner, outputStyle: session.outputStyle, architecture: session.architecture });
  }, [detail?.session.providerId, detail?.session.model, detail?.session.mode, detail?.session.permissionMode, detail?.session.planner?.providerId, detail?.session.planner?.model, detail?.session.outputStyle, detail?.session.architecture?.sidekick.providerId, detail?.session.architecture?.sidekick.model, detail?.session.configRevision, configBusy]);
