...
import { ArrowDown, Check, ChevronRight, Clock3, File, GitFork, Shield, Terminal, Undo2, X } from 'lucide-react';
...
function Approval({ request, onDecide, busy }: { request: PermissionRequest; onDecide: (id: string, decision: 'allow' | 'always' | 'deny') => void; busy: boolean }) {
  return <section className="approval" aria-label="Permission requested"><div className="approval-heading"><span><Shield size={17} /></span><div><strong>A quick check before I continue.</strong><p>{request.description || `${toolLabels[request.tool] || request.tool} needs your permission.`}</p></div></div><details><summary>Review {toolLabels[request.tool]?.toLowerCase() || request.tool}<ChevronRight size={13} /></summary><pre>{JSON.stringify(request.args, null, 2)}</pre></details><div className="approval-actions"><button className="button secondary" disabled={busy} onClick={() => onDecide(request.id, 'deny')}>Deny</button><button className="text-button" title="Remember approval for this tool during the current run" disabled={busy} onClick={() => onDecide(request.id, 'always')}>Always allow this tool</button><button className="button primary" disabled={busy} onClick={() => onDecide(request.id, 'allow')}>Allow once<Check size={14} /></button></div></section>;
}
export function Conversation({ detail, connection, onDecide, onFork, onUndo, busy }: { detail: SessionDetail; connection: 'connecting' | 'connected' | 'reconnecting'; onDecide: (id: string, decision: 'allow' | 'always' | 'deny') => void; onFork: (messageId: string) => void; onUndo: () => void; busy: boolean }) {
  ...
  return <div className="conversation-shell"><div className="conversation-scroll" ref={scroll} onScroll={e => { const el = e.currentTarget; setAtBottom(el.scrollHeight - el.scrollTop - el.clientHeight < 100); }}><div className="conversation-content">
    ...
    {detail.messages.filter(m => m.role !== 'tool').map(message => <MessageView key={message.id} message={message} running={running && message.id === last?.id} onFork={() => onFork(message.id)} onUndo={onUndo} disabled={busy || running} />)}
...
function MessageView({ message, running, onFork, onUndo, disabled }: { message: Message; running: boolean; onFork: () => void; onUndo: () => void; disabled: boolean }) {
  ...
  return <article className={`message ${assistant ? 'assistant-message' : 'user-message'}`} aria-label={assistant ? 'Assistant message' : 'Your message'}>
    ...
    <div className="message-body">{message.reasoning && <details className="thinking"><summary><span className={running ? 'thinking-active' : ''} />Thinking<ChevronRight size={13} /></summary><div className="markdown"><Markdown content={message.reasoning} /></div></details>}
    ...
    {assistant && !running && <div className="message-actions"><CopyButton text={message.content} /><button className="icon-button" onClick={onFork} disabled={disabled} aria-label="Fork session at this message" title="Fork from here"><GitFork size={13} /></button>{message.toolCalls?.some(t => ['write_file', 'edit_file'].includes(t.name) && t.status === 'completed') && <button className="icon-button" onClick={onUndo} disabled={disabled} aria-label="Undo session file changes" title="Undo session file changes"><Undo2 size={13} /></button>}{message.usage && <span className="usage" title="Reported by your model provider">{message.usage.outputTokens.toLocaleString()} tokens{message.usage.durationMs ? ` · ${(message.usage.durationMs / 1000).toFixed(1)}s` : ''}{message.usage.cost !== undefined ? ` · $${message.usage.cost.toFixed(4)}` : ''}</span>}</div>}
  </article>;
}
