...
function SessionApp({ controller, router, onQuit, chooseTheme, themeName, themeMode }: { controller: TerminalController; router: KeymapRouter; onQuit: () => void; chooseTheme: (name: string, mode: 'system' | 'light' | 'dark') => void; themeName: string; themeMode: 'system' | 'light' | 'dark' }) {
  ...
  const commands: MenuItem[] = [
    ...
    { id: 'editor', label: 'Open draft in external editor', description: 'Uses VISUAL or EDITOR, then returns to Speedrail', action: () => { close(); run(() => controller.action('Editing draft', async () => { const text = await editDraft(renderer, controller.getState().draft.text, controller.detail!.session.workspace); controller.setDraft({ ...controller.getState().draft, text }); })); } },
    { id: 'shell', label: 'Open workspace shell', description: 'Type exit to return to Speedrail', action: () => { close(); run(() => openShell(renderer, controller.detail!.session.workspace)); } },
    ...
    { id: 'setup', label: 'Set up Speedrail', description: 'A quick guide to architecture, models, and permissions', disabled: busy, action: () => run(() => openSetup()) },
    ...
    { id: 'quit', label: 'Quit Speedrail TUI', description: 'The server and running tasks keep working', action: onQuit },
  ];
